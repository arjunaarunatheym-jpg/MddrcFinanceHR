...
class PettyCashSetup(BaseModel):
    ...
    approval_threshold: float = 100.0  # Requires approval above this

class PettyCashTransaction(BaseModel):
    type: str  # "topup", "expense"
    amount: float
    description: str
...
async def add_petty_cash_transaction(txn: PettyCashTransaction, current_user: User = Depends(get_current_user)):
    """Add a petty cash transaction (topup or expense)"""
    if current_user.role not in ["admin", "finance"]:
        raise HTTPException(status_code=403, detail="Access denied")
    
    settings = await db.petty_cash_settings.find_one({})
    if not settings:
        raise HTTPException(status_code=400, detail="Petty cash not set up. Please configure settings first.")
    ...
    if txn.type == "expense":
        ...
        raise HTTPException(status_code=400, detail="Invalid transaction type. Use 'expense' or 'topup'")
...
async def approve_petty_cash_transaction(transaction_id: str, current_user: User = Depends(get_current_user)):
    ...
    return {"message": "Transaction approved", "new_balance": new_balance}
...
async def reject_petty_cash_transaction(transaction_id: str, current_user: User = Depends(get_current_user)):
    ...
    await db.petty_cash_transactions.update_one(
        ...
        {"$set": {
            ...
            "rejected_at": get_malaysia_time().isoformat()
        }}
    )
...
async def delete_petty_cash_transaction(transaction_id: str, current_user: User = Depends(get_current_user)):
    ...
    # Only allow deletion of approved transactions - reverse the balance
    if txn.get("status") == "approved":
        settings = await db.petty_cash_settings.find_one({})
        current_balance = settings.get("current_balance", 0)
        
        if txn.get("type") == "expense":
            new_balance = current_balance + txn.get("amount", 0)  # Add back
        elif txn.get("type") == "topup":
            new_balance = current_balance - txn.get("amount", 0)  # Remove
        else:
...
async def reconcile_petty_cash(recon: PettyCashReconciliation, current_user: User = Depends(get_current_user)):
    """Reconcile petty cash - compare physical count with system balance"""
    if current_user.role not in ["admin", "finance"]:
        raise HTTPException(status_code=403, detail="Access denied")
    ...
    # Update balance to physical count (adjust for variance)
    await db.petty_cash_settings.update_one({}, {"$set": {
        "current_balance": recon.physical_count,
        "last_reconciliation": reconciliation["date"]
    }})
    
    # If variance exists, create an adjustment transaction
    if abs(variance) > 0.01:
        adjustment = {
            "id": str(uuid.uuid4()),
            "type": "adjustment",
            "amount": abs(variance),
            "description": f"Reconciliation adjustment ({'+' if variance > 0 else '-'}RM {abs(variance):.2f})",
            "category": "Adjustment",
            "date": get_malaysia_time().isoformat()[:10],
            "notes": f"Variance from reconciliation: {recon.notes or 'No notes'}",
    ...
    return {
        ...
        "physical_count": recon.physical_count,
        "variance": variance,
        "new_balance": recon.physical_count
    }
...
async def get_reconciliation_history(current_user: User = Depends(get_current_user)):
    """Get petty cash reconciliation history"""
    if current_user.role not in ["admin", "finance"]:
        raise HTTPException(status_code=403, detail="Access denied")
